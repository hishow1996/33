...
static int get_fd(JNIEnv *env, jobject self) {
    return (*env)->GetIntField(env, self, g_master);
}

static pid_t get_pid(JNIEnv *env, jobject self) {
    return (pid_t)(*env)->GetLongField(env, self, g_pid);
}
...
Java_com_hishow_terminal33_NativePty_nativeInit(JNIEnv *env, jobject self) {
...
Java_com_hishow_terminal33_NativePty_nativeStart(JNIEnv *env, jobject self, jobjectArray argv, jstring cwd) {
...
Java_com_hishow_terminal33_NativePty_nativeRead(JNIEnv *env, jobject self, jbyteArray buffer) {
...
JNIEXPORT jint JNICALL
Java_com_hishow_terminal33_NativePty_nativeWrite(JNIEnv *env, jobject self, jbyteArray data, jint length) {
    int fd = get_fd(env, self);
    if (fd < 0) return -1;
...
JNIEXPORT void JNICALL
Java_com_hishow_terminal33_NativePty_nativeResize(JNIEnv *env, jobject self, jint rows, jint cols) {
    int fd = get_fd(env, self);
    if (fd < 0) return;
    struct winsize ws;
...
JNIEXPORT void JNICALL
Java_com_hishow_terminal33_NativePty_nativeClose(JNIEnv *env, jobject self) {
    int fd = get_fd(env, self);
    pid_t pid = get_pid(env, self);
    if (fd >= 0) close(fd);
    if (pid > 0) {
