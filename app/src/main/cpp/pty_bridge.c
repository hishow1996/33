...
JNIEXPORT jboolean JNICALL Java_com_hishow_terminal33_NativePty_nativeStart(JNIEnv *env, jobject self, jobjectArray argv, jstring cwd) {
    ...
    for (jsize i = 0; i < argc; i++) {
        ...
        if (!utf) { for (jsize j = 0; j < i; j++) free(args[j]); free(args); return JNI_FALSE; }
        args[i] = strdup(utf);
        (*env)->ReleaseStringUTFChars(env, item, utf);
        (*env)->DeleteLocalRef(env, item);
        if (!args[i]) { for (jsize j = 0; j < i; j++) free(args[j]); free(args); return JNI_FALSE; }
    }
    const char *cwd_utf = (*env)->GetStringUTFChars(env, cwd, NULL);
    if (!cwd_utf) { for (jsize i = 0; i < argc; i++) free(args[i]); free(args); return JNI_FALSE; }
    char *cwd_copy = strdup(cwd_utf);
    (*env)->ReleaseStringUTFChars(env, cwd, cwd_utf);
    if (!cwd_copy) { for (jsize i = 0; i < argc; i++) free(args[i]); free(args); return JNI_FALSE; }

    int master = posix_openpt(O_RDWR | O_NOCTTY | O_CLOEXEC);
    if (master < 0 || grantpt(master) < 0 || unlockpt(master) < 0) {
        if (master >= 0) close(master); free(cwd_copy); for (jsize i = 0; i < argc; i++) free(args[i]); free(args); return JNI_FALSE;
    }
    char *slave_name = ptsname(master);
    if (!slave_name) { close(master); free(cwd_copy); for (jsize i = 0; i < argc; i++) free(args[i]); free(args); return JNI_FALSE; }

    pid_t pid = fork();
    if (pid < 0) { close(master); free(cwd_copy); for (jsize i = 0; i < argc; i++) free(args[i]); free(args); return JNI_FALSE; }
    if (pid == 0) {
    ...
    free(cwd_copy); for (jsize i = 0; i < argc; i++) free(args[i]); free(args);
    return JNI_TRUE;
}

JNIEXPORT jint JNICALL Java_com_hishow_terminal33_NativePty_nativeRead(JNIEnv *env, jobject self, jbyteArray buffer) {
    int fd = get_fd(env, self); if (fd < 0) return -1;
    jsize capacity = (*env)->GetArrayLength(env, buffer); if (capacity <= 0) return 0;
    jbyte *data = (*env)->GetByteArrayElements(env, buffer, NULL); if (!data) return -1;
    ssize_t n = read(fd, data, (size_t)capacity);
    if (n > 0) (*env)->ReleaseByteArrayElements(env, buffer, data, 0); else (*env)->ReleaseByteArrayElements(env, buffer, data, JNI_ABORT);
    return (jint)n;
}
...
JNIEXPORT jint JNICALL Java_com_hishow_terminal33_NativePty_nativeWrite(JNIEnv *env, jobject self, jbyteArray data, jint length) {
    int fd = get_fd(env, self); if (fd < 0 || length <= 0) return -1;
    jsize size = (*env)->GetArrayLength(env, data); if (length > size) length = size;
    jbyte *bytes = (*env)->GetByteArrayElements(env, data, NULL); if (!bytes) return -1;
...
JNIEXPORT void JNICALL Java_com_hishow_terminal33_NativePty_nativeResize(JNIEnv *env, jobject self, jint rows, jint cols) {
    int fd = get_fd(env, self); if (fd < 0) return;
    struct winsize ws = {0}; ws.ws_row = (unsigned short)rows; ws.ws_col = (unsigned short)cols;
    ioctl(fd, TIOCSWINSZ, &ws);
    pid_t pid = get_pid(env, self); if (pid > 0) kill(pid, SIGWINCH);
}

JNIEXPORT void JNICALL Java_com_hishow_terminal33_NativePty_nativeClose(JNIEnv *env, jobject self) {
    int fd = get_fd(env, self); pid_t pid = get_pid(env, self);
    set_fields(env, self, -1, -1);
    if (fd >= 0) close(fd);
    if (pid > 0) { kill(pid, SIGHUP); waitpid(pid, NULL, WNOHANG); }
}
