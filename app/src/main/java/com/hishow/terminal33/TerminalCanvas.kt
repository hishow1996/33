...
class TerminalScreenModel {
    ...
    fun resize(columns: Int, rows: Int) = synchronized(lock) { val c = state.columns; val r = state.rows; state.resize(columns, rows); if (c != state.columns || r != state.rows) version++ }
    fun clear() = synchronized(lock) { state.feed("\u001b[2J\u001b[H"); version++ }
    fun copyText(): String = synchronized(lock) { state.snapshot().joinToString("\n") { row -> row.joinToString("") { it.ch.toString() }.trimEnd() }.trimEnd() }
    fun snapshot() = synchronized(lock) { TerminalFrame(state.snapshot(), state.columns, state.rows, state.cursorX, state.cursorY, state.cursorVisible, version) }
}

data class TerminalFrame(val cells: Array<Array<VtState.Cell>>, val columns: Int, val rows: Int, val cursorX: Int, val cursorY: Int, val cursorVisible: Boolean, val version: Int)
...
fun TerminalCanvas(model: TerminalScreenModel, modifier: Modifier = Modifier, fontSizeSp: Int = 13, onSizeChanged: (rows: Int, columns: Int) -> Unit = { _, _ -> }) {
    ...
    val sizingModifier = modifier.fillMaxSize().onSizeChanged { size ->
        val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply { typeface = android.graphics.Typeface.MONOSPACE; textSize = textSize }
...
private fun xterm256(index: Int): Int {
    if (index < 216) { val r = index / 36; val g = (index / 6) % 6; val b = index % 6; fun level(v: Int) = if (v == 0) 0 else 55 + v * 40; return android.graphics.Color.rgb(level(r), level(g), level(b)) }
    val gray = 8 + (index - 216) * 10; return android.graphics.Color.rgb(gray, gray, gray)
}
