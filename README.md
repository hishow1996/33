...
- PRoot 无 root 运行
- **原生 Linux PTY 会话**：shell 通过 pseudo-terminal 启动，不再使用普通 stdin/stdout 管道
- PTY 支持终端窗口尺寸设置与 SIGWINCH
...
- C.UTF-8、xterm-256color、独立 HOME 环境
- 只构建 arm64-v8a
- Release 开启 R8 与资源压缩
...
当前版本仍会在首次启动时获取 arm64 Android PRoot runtime，并缓存到应用私有目录。下一阶段可以把 PRoot 也做成 APK 内置资源，实现完全离线首次启动。

## PTY 架构

终端会话现在通过 `app/src/main/cpp/pty_bridge.c` 创建 pseudo-terminal，并由 `NativePty.kt` 提供 Kotlin 接口。这样 shell 获得真实的终端设备，而不是普通管道；这为 `vim`、`top`、readline、交互式程序以及更完整的 ANSI/光标行为提供了正确基础。

当前 Compose 输出层仍是轻量文本渲染，ANSI SGR 颜色解析属于下一阶段，暂不宣称已经完整实现。
...
- Kotlin 2.0.21
- Android NDK + CMake（用于 native PTY）
...
这是一个独立的 `33` 项目：UI、初始化流程、Ubuntu 解包逻辑、PTY 和运行管理均独立实现，没有引用其他终端项目。
